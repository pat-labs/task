...
```
*Note: `--network host` is used here to allow the container to easily access the PostgreSQL instance running on localhost.*
...
*   `-d postgres:latest`: Runs the container in detached mode (in the background) using the latest official PostgreSQL image.
